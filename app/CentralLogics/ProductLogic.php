...
class ProductLogic
{
    ...
    public static function format_export_items($foods, $module_type)
    {
        ...
        foreach ($foods as $item) {
            $category_id = 0;
            $sub_category_id = 0;
            foreach ($item->category_ids ?? [] as $key => $category) {
                if ($key == 0) {
            ...
            $storage[] = [
                ...
                'Variations' => json_encode($module_type == 'food' ? ($item->food_variations ?? []) : ($item->variations ?? [])),
                'ChoiceOptions' => json_encode($item?->choice_options ?? []),
                'AddOns' => json_encode($item->add_ons ?? []),
