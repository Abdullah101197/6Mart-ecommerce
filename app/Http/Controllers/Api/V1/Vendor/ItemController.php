...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $item->attributes = $request->has('attribute_id') ? (is_array($request->attribute_id) ? $request->attribute_id : json_decode($request->attribute_id, true)) : [];
        $item->add_ons = $request->has('addon_ids') ? explode(',', $request->addon_ids) : [];
    ...
    public function update(Request $request)
    {
        ...
        $p->choice_options = $choice_options;
        $variations = [];
        $options = [];
        if ($request->has('choice_no')) {
            foreach (json_decode($request->choice_no) as $key => $no) {
        ...
        $variation_changed = false;
        if (
            (!empty($p->food_variations) && !empty($food_variations) && $p->food_variations != $food_variations) || (
                (!empty($p->variations) && !empty($variations) && $p->variations != $variations)
            )
        ) {
            $variation_changed = true;
        }
        ...
        $p->food_variations = $food_variations;
        ...
        $p->attributes = $request->has('attribute_id') ? (is_array($request->attribute_id) ? $request->attribute_id : json_decode($request->attribute_id, true)) : [];
        $p->add_ons = $request->has('addon_ids') ? explode(',', $request->addon_ids) : [];
