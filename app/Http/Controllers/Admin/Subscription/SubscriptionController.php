...
class SubscriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'deliveryman_list' => 'nullable|boolean',
            'employee' => 'nullable|boolean',
            'item' => 'nullable|boolean',
            'banner' => 'nullable|boolean',
            'campaign' => 'nullable|boolean',
            'wallet' => 'nullable|boolean',
            'wallet_method' => 'nullable|boolean',
            'coupon' => 'nullable|boolean',
            'advertisement' => 'nullable|boolean',
            'advertisement_list' => 'nullable|boolean',
            'addon' => 'nullable|boolean',
            'store_setup' => 'nullable|boolean',
            'notification_setup' => 'nullable|boolean',
            'profile' => 'nullable|boolean',
            'my_shop' => 'nullable|boolean',
            'expense_report' => 'nullable|boolean',
            'disbursement_report' => 'nullable|boolean',
            'vat_report' => 'nullable|boolean',
            'ai_pulse' => 'nullable|boolean',
            'omnichannel' => 'nullable|boolean',
            'returns' => 'nullable|boolean',
            'helpcenter' => 'nullable|boolean',
            'dash_kpi_gmv' => 'nullable|boolean',
            'dash_kpi_orders' => 'nullable|boolean',
            'dash_kpi_stock_health' => 'nullable|boolean',
            'dash_kpi_items' => 'nullable|boolean',
            'dash_kpi_tasks_due' => 'nullable|boolean',
            'dash_kpi_ai_pulse_status' => 'nullable|boolean',
            'dash_kpi_nps' => 'nullable|boolean',
            'dash_revenue_trend_channels' => 'nullable|boolean',
            'dash_channel_mix' => 'nullable|boolean',
            'dash_ai_pulse_live' => 'nullable|boolean',
            'dash_recent_orders' => 'nullable|boolean',
            'dash_inventory_alerts' => 'nullable|boolean',
            'dash_today_tasks' => 'nullable|boolean',
            'dash_store_activity' => 'nullable|boolean',
        ...
        $package->category = $request->category ?? 0;
        $package->role = $request->role ?? 0;
        $package->deliveryman_list = $request->deliveryman_list ?? 0;
        $package->employee = $request->employee ?? 0;
        $package->item = $request->item ?? 0;
        $package->banner = $request->banner ?? 0;
        $package->campaign = $request->campaign ?? 0;
        $package->wallet = $request->wallet ?? 0;
        $package->wallet_method = $request->wallet_method ?? 0;
        $package->coupon = $request->coupon ?? 0;
        $package->advertisement = $request->advertisement ?? 0;
        $package->advertisement_list = $request->advertisement_list ?? 0;
        $package->addon = $request->addon ?? 0;
        $package->store_setup = $request->store_setup ?? 0;
        $package->notification_setup = $request->notification_setup ?? 0;
        $package->profile = $request->profile ?? 0;
        $package->my_shop = $request->my_shop ?? 0;
        $package->expense_report = $request->expense_report ?? 0;
        $package->disbursement_report = $request->disbursement_report ?? 0;
        $package->vat_report = $request->vat_report ?? 0;
        $package->ai_pulse = $request->ai_pulse ?? 0;
        $package->omnichannel = $request->omnichannel ?? 0;
        $package->returns = $request->returns ?? 0;
        $package->helpcenter = $request->helpcenter ?? 0;
        $package->dash_kpi_gmv = $request->dash_kpi_gmv ?? 0;
        $package->dash_kpi_orders = $request->dash_kpi_orders ?? 0;
        $package->dash_kpi_stock_health = $request->dash_kpi_stock_health ?? 0;
        $package->dash_kpi_items = $request->dash_kpi_items ?? 0;
        $package->dash_kpi_tasks_due = $request->dash_kpi_tasks_due ?? 0;
        $package->dash_kpi_ai_pulse_status = $request->dash_kpi_ai_pulse_status ?? 0;
        $package->dash_kpi_nps = $request->dash_kpi_nps ?? 0;
        $package->dash_revenue_trend_channels = $request->dash_revenue_trend_channels ?? 0;
        $package->dash_channel_mix = $request->dash_channel_mix ?? 0;
        $package->dash_ai_pulse_live = $request->dash_ai_pulse_live ?? 0;
        $package->dash_recent_orders = $request->dash_recent_orders ?? 0;
        $package->dash_inventory_alerts = $request->dash_inventory_alerts ?? 0;
        $package->dash_today_tasks = $request->dash_today_tasks ?? 0;
        $package->dash_store_activity = $request->dash_store_activity ?? 0;
    ...
    public function update(SubscriptionPackage $subscriptionackage, Request $request)
    {
        ...
        $subscriptionackage->category = $request->category ?? 0;
        $subscriptionackage->role = $request->role ?? 0;
        $subscriptionackage->deliveryman_list = $request->deliveryman_list ?? 0;
        $subscriptionackage->employee = $request->employee ?? 0;
        $subscriptionackage->item = $request->item ?? 0;
        $subscriptionackage->banner = $request->banner ?? 0;
        $subscriptionackage->campaign = $request->campaign ?? 0;
        $subscriptionackage->wallet = $request->wallet ?? 0;
        $subscriptionackage->wallet_method = $request->wallet_method ?? 0;
        $subscriptionackage->coupon = $request->coupon ?? 0;
        $subscriptionackage->advertisement = $request->advertisement ?? 0;
        $subscriptionackage->advertisement_list = $request->advertisement_list ?? 0;
        $subscriptionackage->addon = $request->addon ?? 0;
        $subscriptionackage->store_setup = $request->store_setup ?? 0;
        $subscriptionackage->notification_setup = $request->notification_setup ?? 0;
        $subscriptionackage->profile = $request->profile ?? 0;
        $subscriptionackage->my_shop = $request->my_shop ?? 0;
        $subscriptionackage->expense_report = $request->expense_report ?? 0;
        $subscriptionackage->disbursement_report = $request->disbursement_report ?? 0;
        $subscriptionackage->vat_report = $request->vat_report ?? 0;
        $subscriptionackage->ai_pulse = $request->ai_pulse ?? 0;
        $subscriptionackage->omnichannel = $request->omnichannel ?? 0;
        $subscriptionackage->returns = $request->returns ?? 0;
        $subscriptionackage->helpcenter = $request->helpcenter ?? 0;
        $subscriptionackage->dash_kpi_gmv = $request->dash_kpi_gmv ?? 0;
        $subscriptionackage->dash_kpi_orders = $request->dash_kpi_orders ?? 0;
        $subscriptionackage->dash_kpi_stock_health = $request->dash_kpi_stock_health ?? 0;
        $subscriptionackage->dash_kpi_items = $request->dash_kpi_items ?? 0;
        $subscriptionackage->dash_kpi_tasks_due = $request->dash_kpi_tasks_due ?? 0;
        $subscriptionackage->dash_kpi_ai_pulse_status = $request->dash_kpi_ai_pulse_status ?? 0;
        $subscriptionackage->dash_kpi_nps = $request->dash_kpi_nps ?? 0;
        $subscriptionackage->dash_revenue_trend_channels = $request->dash_revenue_trend_channels ?? 0;
        $subscriptionackage->dash_channel_mix = $request->dash_channel_mix ?? 0;
        $subscriptionackage->dash_ai_pulse_live = $request->dash_ai_pulse_live ?? 0;
        $subscriptionackage->dash_recent_orders = $request->dash_recent_orders ?? 0;
        $subscriptionackage->dash_inventory_alerts = $request->dash_inventory_alerts ?? 0;
        $subscriptionackage->dash_today_tasks = $request->dash_today_tasks ?? 0;
        $subscriptionackage->dash_store_activity = $request->dash_store_activity ?? 0;
