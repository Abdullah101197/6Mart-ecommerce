...
@php
  ...
  $language = $language ?? getWebConfig('language');
  $languages = $languages ?? (is_array($language) ? $language : []);
  $defaultLang = $defaultLang ?? str_replace('_', '-', app()->getLocale());
  ...
  // Wizard context adapter:
  // - Admin stays on this view.
  // - Vendor will reuse this wizard via a wrapper view (Phase 3+).
  // - Keep a single source of truth for URLs + feature flags.
  $npWizardContext = $npWizardContext ?? (request('np_context') ?: (Auth::guard('vendor')->check() ? 'vendor' : 'admin'));
  $npWizardContext = in_array($npWizardContext, ['admin', 'vendor'], true) ? $npWizardContext : 'admin';
  $npWizardStoreId = $npWizardContext === 'vendor' ? \App\CentralLogics\Helpers::get_store_id() : null;
  $npWizardStoreName = $npWizardContext === 'vendor' ? \App\Models\Store::where('id', $npWizardStoreId)->value('name') : null;

  $npWizardUrls = [
    'list' => $npWizardContext === 'vendor' ? route('vendor.item.list') : route('admin.item.list'),
    'pendingList' => $npWizardContext === 'vendor' ? route('vendor.item.pending_item_list') : null,
    'store' => $npWizardContext === 'vendor' ? route('vendor.item.store') : route('admin.item.store'),
    'updateBase' => $npWizardContext === 'vendor' ? url('vendor-panel/item/update') : url('admin/item/update'),
    'editBase' => $npWizardContext === 'vendor' ? route('vendor.item.edit', [0]) : route('admin.item.edit', [0]),
    'categories' => $npWizardContext === 'vendor' ? route('vendor.item.get-categories') : route('admin.item.get-categories'),
    'variantCombination' => $npWizardContext === 'vendor' ? route('vendor.item.variant-combination') : route('admin.item.variant-combination'),
    'productGallery' => $npWizardContext === 'vendor' ? route('vendor.item.product_gallery') : route('admin.item.product_gallery'),
    'reviews' => $npWizardContext === 'vendor' ? null : route('admin.item.reviews'),

    // Admin-only create-from-modal endpoints. Vendor should not see these controls.
    'brandStore' => $npWizardContext === 'vendor' ? null : route('admin.brand.store'),
    'categoryStoreBase' => $npWizardContext === 'vendor' ? null : route('admin.item.category.store'),
    'unitStore' => $npWizardContext === 'vendor' ? null : route('admin.item.unit.store'),
    'instacartOptionsStore' => $npWizardContext === 'vendor' ? null : route('admin.item.instacart.options.store'),
    'itemTypesStore' => $npWizardContext === 'vendor' ? null : route('admin.item.item-types.store'),
    'attributeAjaxStore' => $npWizardContext === 'vendor' ? null : route('admin.attribute.ajax-store'),
    'productSelectOptionsStore' => $npWizardContext === 'vendor' ? null : route('admin.item.product-select-options.store'),
  ];

  $npWizardFeatures = [
    'canManageReviews' => $npWizardContext !== 'vendor',
    'canCreateFromModal' => $npWizardContext !== 'vendor',
    'storeMode' => $npWizardContext === 'vendor' ? 'forced' : 'selectable',
  ];
@endphp
<div class="content container-fluid">
  ...
  <div class="page-header d-flex flex-wrap __gap-15px justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex align-items-center" style="gap:10px">
      ...
      <a href="{{ $npWizardUrls['productGallery'] }}"
  ...
  <form id="item_form" enctype="multipart/form-data" class="custom-validation new-product-form" data-ajax="true">
    <input type="hidden" id="request_type" value="{{ $npWizardContext }}">
            ...
            <div class="np-card-body">
              ...
              <div class="np-form-row">
                <div class="np-form-group">
                  ...
                  @if($npWizardContext === 'vendor')
                    <input type="hidden" name="store_id" id="store_id" value="{{ old('store_id', $product?->store_id ?? $npWizardStoreId) }}">
                    <input type="text" class="np-input" value="{{ $npWizardStoreName ?? '-' }}" disabled>
                  @else
                ...
                <?php foreach(($npReviewStats['latest'] ?? collect()) as $rev): ?>
                  ...
                  <div class="np-review-card">
                    ...
                    <div class="np-review-actions">
                      ...
                      @if($npWizardFeatures['canManageReviews'])
                        <a class="np-btn-tiny" href="{{ $npWizardUrls['reviews'] }}" target="_blank" rel="noopener">Manage</a>
                      @endif
                    </div>
                  </div>
                <?php endforeach; ?>
                @if($npWizardFeatures['canManageReviews'])
                  <a class="np-btn-add" style="margin-top:10px;display:inline-flex" href="{{ $npWizardUrls['reviews'] }}" target="_blank"
    ...
    <div class="np-action-bar">
      ...
      <div class="np-btn-row">
        <a href="{{ $npWizardUrls['list'] }}" class="np-btn np-btn-ghost">Cancel</a>
...
@push('script_2')
  <script>
    ...
    // ΓöÇΓöÇ Custom tabs (builder) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    let npCustomTabs = [];
    ...
    function npAddSection(tabId) {
      ...
      if (!title) return;
      const icon = (prompt('Section icon (emoji)', '≡ƒôî') || '').trim() || '≡ƒôî';
    ...
    function npRenderCustomTabs() {
      ...
      (npCustomTabs || []).forEach(tab => {
        ...
        if (nav && addBtn) {
          ...
          btn.textContent = `${tab.icon || '≡ƒº⌐'} ${tab.title || 'Custom'}`;
        ...
        const sectionHtml = sections.map(sec => {
          ...
          return `
              <div class="np-card" style="margin-top:12px">
                <div class="np-card-header">
                  <span class="np-card-icon">${sec.icon || '≡ƒôî'}</span>
                ...
                <div class="np-card-body">
                  ...
                  ${fieldsHtml || '<div class="np-hint">No fields yet. Click ΓÇ£+ Add FieldΓÇ¥.</div>'}
                </div>
              </div>
            `;
        }).join('');
        ...
        panel.innerHTML = `
            <div class="np-grid">
              <div>
                <div class="np-card">
                  <div class="np-card-header">
                    <span class="np-card-icon">${tab.icon || '≡ƒº⌐'}</span>
                  ...
                  <div class="np-card-body">
                    ...
                    ${sectionHtml || '<div class="np-hint">No sections yet. Click ΓÇ£+ Add SectionΓÇ¥.</div>'}
                  </div>
                </div>
              </div>
              <div></div>
    ...
    function npApplyTabsUi() {
      ...
      const map = {
        'np-tab-general': { defaultTitle: '≡ƒô¥ General' },
        'np-tab-attributes': { defaultTitle: '≡ƒöº Attributes' },
        'np-tab-nutrition': { defaultTitle: '≡ƒº¬ Nutrition & Allergens' },
        'np-tab-variants': { defaultTitle: '≡ƒÄ¿ Variants' },
        'np-tab-seo': { defaultTitle: '≡ƒöì SEO & Meta' },
        'np-tab-media': { defaultTitle: '≡ƒû╝∩╕Å Media' },
        'np-tab-logistics': { defaultTitle: 'Γ£ê∩╕Å Logistics & Shipping' },
        'np-tab-reviews': { defaultTitle: 'Γ¡É Reviews & Analytics' },
      };
      ...
      // Built-in panels: keep hidden tabs out of the layout and inactive (matches Manage Tabs ΓåÆ Visible)
    ...
    function npRenderBuiltInAddedSections() {
      ...
      added.forEach(sec => {
        ...
        card.innerHTML = `
            <div class="np-card-header">
              <span class="np-card-icon">${sec.icon || '≡ƒº⌐'}</span>
    ...
    function npAddBuiltInSection(tabId) {
      const title = (prompt('Section title', 'New Section') || '').trim();
      if (!title) return;
      const icon = (prompt('Section icon (emoji)', '≡ƒº⌐') || '').trim() || '≡ƒº⌐';
