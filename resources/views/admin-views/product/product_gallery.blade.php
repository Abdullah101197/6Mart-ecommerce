...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="stats-bar">
            <div class="stat-card active-filter" onclick="filterByStatus('all', this)">
                ...
                <div>
                    <div class="stat-val" id="sv-total">{{ $total_count }}</div>
            ...
            <div class="stat-card" onclick="filterByStatus('active', this)">
                ...
                <div>
                    <div class="stat-val" id="sv-active">{{ $active_count }}</div>
            ...
            <div class="stat-card" onclick="filterByStatus('draft', this)">
                ...
                <div>
                    <div class="stat-val" id="sv-draft">{{ $draft_count }}</div>
            ...
            <div class="stat-card" onclick="filterByStatus('oos', this)">
                ...
                <div>
                    <div class="stat-val" id="sv-oos">{{ $oos_count }}</div>
            ...
            <div class="stat-card" onclick="filterByStatus('expiry', this)">
                ...
                <div>
                    <div class="stat-val" id="sv-expiry">{{ $expiry_count }}</div>
            ...
            <form id="gallery-filter-form">
                <div class="filter-row">
                    <div class="search-wrap">
                        ...
                        <input type="text" name="search" id="gallery-search" value="{{ request()?->search ?? '' }}"
                            placeholder="Search product name, SKU, or EAN..." oninput="debouncedSearch()">
                    </div>
                    <div class="filter-group">
                        <span class="filter-label">{{ translate('messages.Category') }}</span>
                        <select name="category_id" id="category_id" class="filter-sel" style="min-width: 150px;"
                            onchange="applyFilters()">
                            <option value="all">{{ translate('messages.all_categories') }}</option>
                            @foreach($categories as $cat)
                                <option value="{{$cat->id}}" {{ (isset($category) && $category->id == $cat->id) || request('category_id') == $cat->id ? 'selected' : '' }}>{{$cat->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="filter-group">
                        <span class="filter-label">{{ translate('messages.Brand') }}</span>
                        <select name="brand_id" id="brand_id" class="filter-sel" style="min-width: 150px;"
                            onchange="applyFilters()">
                            <option value="all">{{ translate('messages.all_brands') }}</option>
                    ...
                    <div class="filter-group">
                        <span class="filter-label">{{ translate('messages.Status') }}</span>
                        <select name="status_filter" id="filter-status-select" class="filter-sel" style="min-width: 150px;"
                            onchange="applyFilters()">
                            <option value="all">{{ translate('messages.all_status') }}</option>
                            <option value="active" {{ request('status_filter') == 'active' ? 'selected' : '' }}>
                                {{ translate('messages.Active') }}
                            </option>
                            <option value="draft" {{ request('status_filter') == 'draft' ? 'selected' : '' }}>
                                {{ translate('messages.Draft') }}
                            </option>
                            <option value="oos" {{ request('status_filter') == 'oos' ? 'selected' : '' }}>Out of Stock
                            </option>
                            <option value="expiry" {{ request('status_filter') == 'expiry' ? 'selected' : '' }}>Near Expiry
                            </option>
                        </select>
                    </div>
                    <div class="filter-group">
                        <span class="filter-label">{{ translate('messages.VAT') }}</span>
                        <select name="vat" class="filter-sel" style="min-width: 150px;" onchange="applyFilters()">
                            <option value="all">{{ translate('Any VAT') }}</option>
                            <option value="0" {{ request('vat') == '0' ? 'selected' : '' }}>0% (Exempt)</option>
                            <option value="5" {{ request('vat') == '5' ? 'selected' : '' }}>5% Standard</option>
                        </select>
                    </div>
                    <div class="filter-divider"></div>
                    <div class="filter-group">
                        <span class="filter-label">{{ translate('Price') }}</span>
        ...
        <div class="table-wrap">
            <div class="table-header">
                <div style="font-size:13px; color:var(--muted)" class="table-count">
                    Showing <strong id="showingCount">{{ $items->count() }}</strong> of <strong
                        id="totalCount">{{ $total_count }}</strong> products
                </div>
                <div style="display:flex;align-items:center;gap:12px">
                    <select class="filter-sel" onchange="$('#filter-limit').val(this.value); applyFilters()"
                        ...
                        <option value="20" {{ request('limit') == 20 ? 'selected' : '' }}>20 / page</option>
                    ...
                    <select class="filter-sel" onchange="$('#filter-sort').val(this.value); applyFilters()"
                        ...
                        <option value="price_asc" {{ request('sort_by') == 'price_asc' ? 'selected' : '' }}>Price: Low to
                            High</option>
                        <option value="price_desc" {{ request('sort_by') == 'price_desc' ? 'selected' : '' }}>Price: High
                            to Low</option>
                        <option value="stock_desc" {{ request('sort_by') == 'stock_desc' ? 'selected' : '' }}>Stock: High
                            to Low</option>
                        <option value="margin" {{ request('sort_by') == 'margin' ? 'selected' : '' }}>Margin: High to Low
                        </option>
                        <option value="expiry" {{ request('sort_by') == 'expiry' ? 'selected' : '' }}>Expiry: Soonest
                            first</option>
                    </select>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table id="product-table">
                        ...
                        <tr>
                            ...
                            <th
                                ...
                                Margin <span class="sort-icon">↕</span> <span
                                    title="{{ translate('Margin is calculated as ((Price - Cost) / Price) * 100') }}"
                            ...
                            <th
                                ...
                                Min. Shelf Life <span class="sort-icon">↕</span> <span
                                    title="{{ translate('Days remaining until product expiry') }}"
                                    style="cursor:help; font-size:10px;">❓</span></th>
                            <th>VAT</th>
                    ...
                    <tbody id="set-rows">
                        @include('admin-views.product.partials._gallery', ['items' => $items])
                    </tbody>
                </table>
            </div>
            ...
            <div class="g-modal-body">
                ...
                <div class="form-group">
                    ...
                    <select class="fi" id="discountType" onchange="updateDiscountPreview()">
                        ...
                        <option value="amount">Fixed Amount Off ({{ \App\CentralLogics\Helpers::currency_symbol() }})
                        </option>
                    </select>
                </div>
                <div class="form-group">
    ...
    <div class="g-modal-overlay" id="importModal">
        <div class="g-modal">
            ...
            <form id="importForm" method="POST" action="{{ route('admin.item.bulk-import') }}"
                ...
                <div class="g-modal-footer">
                    <a href="{{ route('admin.item.bulk-export-index', ['module_id' => Config::get('module.current_module_id')]) }}"
...
@push('script_2')
    <script>
        ...
        function applyFilters(page = 1) {
            ...
            $.ajax({
                url: '{{ route("admin.item.search") }}?page=' + page,
                method: 'POST',
                data: formData,
        ...
        function updateActiveFilterChips() {
            ...
            if (cat && cat !== 'All Categories' && cat !== '' && cat !== '{{ translate("messages.all_categories") }}') chips.push('Category: ' + cat);
            if (brand && brand !== 'All Brands' && brand !== '' && brand !== '{{ translate("messages.all_brands") }}') chips.push('Brand: ' + brand);
        ...
        function bulkRequest(action) {
            ...
            confirm_alert('bulk-action', 'Are you sure you want to perform this action?').then(result => { if (result.value) { $.post('{{ url("admin/item/bulk/action") }}/' + action, { _token: '{{ csrf_token() }}', id: Array.from(selectedIds) }, function (res) { toastr.success(res.message); clearSelection(); applyFilters(); }).fail(function (err) { toastr.error(err.responseJSON.message || 'Action failed'); }); } });
        }
        ...
        function editField(id, field, currentVal, label) {
            ...
            if (field === 'price') {
                ...
                bodyHtml = `
                                                    <div class="form-group">
                                                        <label class="ml" for="ie-cost">Cost Price ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
                                                    ...
                                                    <div class="form-group">
                                                        <label class="ml" for="ie-sell">Sell Price ({{ \App\CentralLogics\Helpers::currency_symbol() }})</label>
        ...
        function calcMarginPreview() {
            ...
            $('#ie-final').text('{{ \App\CentralLogics\Helpers::currency_symbol() }} ' + final.toFixed(2));
        ...
        $(function () {
                ...
                $('#store_id').select2({
                    ajax: {
                        ...
                        data: function (params) {
                            return { q: params.term, module_id: '{{ $moduleId }}', page: params.page };
                        },
            ...
            $('#quickEditForm').on('submit', function (e) {
                ...
                let payload = { _token: '{{ csrf_token() }}', id: id };
                ...
                $.post('{{ route("admin.item.stock-update") }}', payload, function (res) {
        ...
        function applyBulkDiscount() {
            ...
            if (confirm('Apply ' + type + ' discount of ' + amount + ' to ' + selectedIds.size + ' product(s)?')) {
                $.post('{{ route("admin.item.bulk-action", ["action" => "discount"]) }}', {
                    _token: '{{ csrf_token() }}',
        ...
        function exportData() {
            let formData = $('#gallery-filter-form').serialize();
            let url = '{{ route("admin.item.export") }}?' + formData;
